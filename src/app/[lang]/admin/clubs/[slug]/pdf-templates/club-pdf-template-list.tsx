...
async function createClubPdfTemplate(slug: string, name: string, description: string, file: File) {
...
async function loadClubPdfTemplatesList(slug: string, dispatchData: React.Dispatch<DataAction>) {
  try {
    const res = await fetch(`/api/clubs/${slug}/pdf-templates`)
    const data = await res.json()
...
async function deleteClubPdfTemplate(templateId: string) {
  const res = await fetch(`/api/admin/pdf-templates/${templateId}`, { method: "DELETE" })
  if (!res.ok) throw new Error("Delete failed")
}

async function uploadClubPdfTemplate(slug: string, name: string, description: string, file: File) {
  try {
    const template = await createClubPdfTemplate(slug, name, description, file)
...
async function deleteClubPdfTemplateById(templateId: string) {
  try {
    await deleteClubPdfTemplate(templateId)
...
function dataReducer(state: DataState, action: DataAction): DataState {
  switch (action.type) {
    ...
    case "DELETE_SUCCESS":
      return { ...state, templates: state.templates.filter((tpl) => tpl.id !== action.templateId), deleting: null }
    case "STOP_DELETING":
...
export function ClubPdfTemplateList() {
  ...
  useEffect(() => {
    loadClubPdfTemplatesList(slug, dispatchData)
  }, [slug])
  ...
  async function handleDelete(templateId: string) {
    ...
    const result = await deleteClubPdfTemplateById(templateId)
    if (result.success) {
